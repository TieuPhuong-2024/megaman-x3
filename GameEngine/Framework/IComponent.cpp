...
void Movement::setVx(float vx)
{
	this->_velocity.x = vx;
}

void Movement::setVy(float vy)
{
	this->_velocity.y = vy;
}

void Movement::setAccelx(float ax)
{
	this->_accelerate.x = ax;
}

void Movement::setAccely(float ay)
{
	this->_accelerate.y = ay;
}
...
void Gravity::update(float deltatime)
{
	switch (_status)
	{
	case eGravityStatus::FALLING__DOWN:
		this->_additionalVeloc = this->_gravity * deltatime / 1000;
		break;
	case eGravityStatus::SHALLOWED:
...
void Gravity::setgx(float gx)
{
	_gravity.x = gx;
}

void Gravity::setgy(float gy)
{
	_gravity.y = gy;
}
...
Gravity::Gravity(GVector2 gravity, Movement* movement)
{
	...
	this->_status = eGravityStatus::FALLING__DOWN;
}
#pragma endregion
